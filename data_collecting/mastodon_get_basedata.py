...
def get_jwt(api_key):
    ...
    if res.ok:
        ...
        raise Exception(f"get JWT failure: {res.status_code}, {res.text}")
...
def initialize_base_data():
    ...
    print("📦 initialize basedata...")

    # Obtain JWT certification
    jwt = get_jwt(API_KEY)

    # Set the search statement for the base date
    base_query = 'text:"tariff" AND date:[2025-04-01 TO 2025-05-10]'

    # Pull all the matching results from the ElasticSearch API
    ids = fetch_all_ids(jwt, base_query)
    print(f"🔢 total ID number: {len(ids)}")

    # Regular expression: Match the URL format of Mastodon posts
    mastodon_post_pattern = re.compile(
        r'^(?:https?://)?[a-zA-Z0-9.-]+/(?:@[^/]+|users?/[^/]+)/\d+$'
    )

    # Only retain the links of legal Mastodon posts
    ...
    # Store it in the global variable "data" collection to prevent duplicate processing
    data.update(valid_ids)

    # Traverse each URL in the data and call the Mastodon API to obtain the detailed content
    for i, id in enumerate(valid_ids):
        try:
            print(f"⏳ [{i+1}/{len(valid_ids)}] processing: {id}")
            json_data.append(process_url(id))
            time.sleep(1)  # Prevent API call rate limiting
...
def process_url(url):
    actual_url = 'https://' + url.strip().replace('"', '')
...
def main():
    initialize_base_data()  # Initialize data and json_data first
